...
// 前提: index.html の最後で UMD の sanityClient を読み込んでから、このファイルを読み込むこと。
...
const SANITY = {
  ...
  token:     undefined,    // dataset が Public(read) なら不要
  useCdn:    true,
};

// ---- クライアント生成（グローバルに置く）----
const client = (window.sanityClient && window.sanityClient.createClient)
  ? window.sanityClient.createClient(SANITY)
  : null;
...
// ===== GROQ =====
const qModels   = `*[_type == "model"]|order(order asc){_id,name,role,profile,youtubeUrl,links{spotify,applemusic,deezer,amazonmusic},"imageUrl": coalesce(imageUrl, image.asset->url, "")}`;
...
(function init(){
  ...
  Promise.all([renderModels(), renderServices(), renderNews()])
    .catch(e => console.error("[CMS] init error:", e));
})();
